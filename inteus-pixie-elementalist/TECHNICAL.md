...
HotA loads the copies in `Data/H3ab_spr.lod` after the base resources in
`Data/H3sprite.lod`. Patching only the base archive therefore leaves the
visible expansion resources unchanged.

The patcher:

1. Extracts `TwCrPort.def`, `UN32.def`, and `UN44.def` from
   `Data/H3ab_spr.lod`.
...
No other hero's specialty frame is changed. `Data/H3sprite.lod` remains
untouched.
...
- DEF differences restricted to frame 140 in `UN32.def` and `UN44.def`
- Visual verification of the generated 32x32 and 44x44 Pixie frames
- `Data/H3sprite.lod` remained at its original SHA-256 hash
